...
CGameObject* CGameObject::FindFrame(_TCHAR* pstrFrameName)
{
	...
	if (!_tcsncmp(m_pstrFrameName, pstrFrameName, max(_tcslen(m_pstrFrameName), _tcslen(pstrFrameName)))) return(this);
	...
	LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale)
{
	...
	m_nTerrainBlocks = cxBlocks * czBlocks;
	m_ppTerrainBlocks = new CGameObject * [m_nTerrainBlocks];
	for (int i = 0; i < m_nTerrainBlocks; i++) m_ppTerrainBlocks[i] = NULL;

	for (int z = 0; z < czBlocks; z++)
	{
		for (int x = 0; x < cxBlocks; x++)
		{
			int xStart = x * (nBlockWidth - 1);
			int zStart = z * (nBlockLength - 1);

			CGameObject* pBlock = new CGameObject();
			pBlock->SetMesh(new CHeightMapGridMesh(pd3dDevice, pd3dCommandList, xStart, zStart, nBlockWidth, nBlockLength, xmf3Scale, m_pHeightMapImage));

			pBlock->m_nMaterials = 1;
			pBlock->m_ppMaterials = new CMaterial * [1];
			pBlock->m_ppMaterials[0] = new CMaterial();
			pBlock->m_ppMaterials[0]->SetPseudoLightingShader();
			m_ppTerrainBlocks[x + (z * cxBlocks)] = pBlock;
		}
	}
}
...
{
	if (m_ppTerrainBlocks)
	{
		for (int i = 0; i < m_nTerrainBlocks; i++) if (m_ppTerrainBlocks[i]) delete m_ppTerrainBlocks[i];
		delete[] m_ppTerrainBlocks;
	}
...
{
	for (int i = 0; i < m_nTerrainBlocks; i++) if (m_ppTerrainBlocks[i]) m_ppTerrainBlocks[i]->Render(pd3dCommandList, pCamera);
}

void CHeightMapTerrain::ReleaseUploadBuffers()
{
	for (int i = 0; i < m_nTerrainBlocks; i++) if (m_ppTerrainBlocks[i]) m_ppTerrainBlocks[i]->ReleaseUploadBuffers();
}